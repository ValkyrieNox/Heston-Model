...
def parse_args():
    ...
    p.add_argument("--w-sigmmd", type=float, default=0.0)
    p.add_argument("--w-sigw1", type=float, default=0.0)
...
def main():
    ...
    (run_dir / "config.json").write_text(json.dumps({
        "config": asdict(cfg), "w_sigmmd": a.w_sigmmd, "w_sigw1": a.w_sigw1, "w_energy": a.w_energy,
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for _ in range(cfg.steps_per_epoch):
            ...
            if a.w_sigmmd > 0 or a.w_sigw1 > 0:
                rf = signature_features(real_norm, a.sig_depth); ff = signature_features(fake_norm, a.sig_depth)
                if a.w_sigmmd > 0: pl = pl + a.w_sigmmd * loss_sig_mmd(rf, ff)
                if a.w_sigw1 > 0: pl = pl + a.w_sigw1 * loss_sig_w1(rf, ff)
            if a.w_energy > 0:
                pl = pl + a.w_energy * loss_energy(real_norm, fake_norm)
            ...
            loss = pl + moment + cfg.anchor_weight * anchor
        ...
        for name, model, src, stage in (("vol", vol_model, vol_ckpt, "vol"), ("ret", ret_model, ret_ckpt, "ret")):
            extra = {"pathwise_finetuned": True, "loss": "combined", "kind": "fm", "pathwise_score": score}
        ...
        print(f"[combined] epoch {epoch}/{cfg.epochs} G={rec['generator_loss']:.5f} "
    ...
    print(f"[combined] done {run_dir}", flush=True)
