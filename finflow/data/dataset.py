...
def _one_hot_matrix(actions: np.ndarray, num_actions: int) -> np.ndarray:
    """Vectorized one-hot encoding for an int action array."""
    actions = np.asarray(actions, dtype=np.int64)
    if num_actions <= 0:
        raise ValueError("num_actions must be positive")
    if actions.size > 0 and (actions.min() < 0 or actions.max() >= num_actions):
        raise IndexError(f"action indices out of range [0, {num_actions})")
    out = np.zeros((actions.shape[0], num_actions), dtype=np.float32)
